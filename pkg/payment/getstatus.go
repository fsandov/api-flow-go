...
const (
	...
	getStatusByFlowOrderURL  = "/payment/getStatusByFlowOrder"
)
...
func (c *Client) GetStatusByCommerceID(ctx context.Context, commerceID string) (*Status, error) {
	if commerceID == "" {
		return nil, fmt.Errorf("commerceID is required")
	}
...
// GetStatusFlowOrder gets the status of an order in Flow.cl by flowOrder. It returns a GetStatusResponse and an error if any.
func (c *Client) GetStatusFlowOrder(ctx context.Context, flowOrder string) (*Status, error) {
	if flowOrder == "" {
		return nil, fmt.Errorf("flowOrder is required")
	}

	dataString := utils.SortFields(StatusURIData{APIKey: c.APIKey, FlowOrder: flowOrder})
	signature, err := utils.GenerateHMAC(dataString, c.SecretKey)
	if err != nil {
		return nil, fmt.Errorf("error generating hmac. err: %v", err)
	}
	encode := url.Values{}
	encode.Set("apiKey", c.APIKey)
	encode.Set("flowOrder", flowOrder)
	encodeString := encode.Encode()

	URI := fmt.Sprintf("%s%s?%s&s=%s", c.BaseURL, getStatusByFlowOrderURL, encodeString, signature)
	req, _ := http.NewRequestWithContext(ctx, http.MethodGet, URI, nil)
	res, err := c.HTTPClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("error making request. err: %v", err)
